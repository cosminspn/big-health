/**
 * 
 */
package ro.bighealth.commons.service;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.context.annotation.Scope;
import org.springframework.context.annotation.ScopedProxyMode;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import ro.bighealth.commons.core.BaseRepository;
import ro.bighealth.commons.core.BaseService;
import ro.bighealth.commons.entity.Disease;
import ro.bighealth.commons.repository.DiseaseRepository;

/**
 * Service for {@link Disease} entity.
 * 
 * @author CosminS
 * @since Mar 6, 2014
 */
@Service
@Scope(proxyMode = ScopedProxyMode.TARGET_CLASS)
@Transactional(readOnly = true, rollbackFor = Exception.class)
public class DiseaseService extends BaseService<Disease> {

	/**
	 * Injected {@link DiseaseRepository} repository.
	 */
	@Autowired
	private DiseaseRepository diseaseRepository;

	@Override
	protected BaseRepository<Disease> getRepository() {
		return diseaseRepository;
	}
}